...
//            val data = Uri.parse(dataString)
            if (data.scheme.equals("mobilesign")) {
                checkP12List(data)
            } else if (data.scheme.equals("content")) {
//                account_type=com.google&mimeType=application%2Fpkix-cert&rendition=1
//                account_type=com.google&mimeType=application%2Foctet-stream&rendition=1
                requireActivity().contentResolver.openInputStream(data)?.let {
                    ...
                    if(data.encodedQuery?.contains("pkix-cert") == true){
                        fileDownload = ImportHelper.writeTempFileCer(
                            requireContext(),
                            it
                        )
                    }else if(data.encodedQuery?.contains("octet-stream") == true){
                        fileDownload = ImportHelper.writeTempFileP7b(
                            requireContext(),
                            it
                        )
                    }else{
                    ...
                    val action =
                        ImportKeyFragmentDirections.nextActionCsrList(fileDownload.path)
...
//        }
        ...
        viewBinding.apply {
            ...
            btnCsr.setOnClickListener {
                val action =
                    ImportKeyFragmentDirections.nextActionCsrCreate()
                findNavController().navigate(action)
            }
            ...
            btnCsrList.setOnClickListener {
                val action =
                    ImportKeyFragmentDirections.nextActionCsrList("")
                findNavController().navigate(action)
            }
            btnCsrList2.setOnClickListener {
                val action =
                    ImportKeyFragmentDirections.nextActionCsrList("")
                findNavController().navigate(action)
            }
...
//                            Log.i("private",adapterCert.getItem(position).certName)
                        }
    ...
    fun alertDelete(certificate: Certificate, position: Int) {
        certificate.certCa?.let { Log.i("p12", it) }
