...
//                    val privateKey: Key = p12.getKey(alias, password)
        ...
        fun writeTempFileCer(context: Context, input: InputStream): File {
            try {
                val file = File(context.getCacheDir(), "temp_cer.cer")
                FileOutputStream(file).use { output ->
                    val buffer = ByteArray(4 * 1024) // or other buffer size
                    var read: Int
                    while (input.read(buffer).also { read = it } != -1) {
        ...
        fun writeTempFileP7b(context: Context, input: InputStream): File {
            try {
                val file = File(context.getCacheDir(), "temp_chains.p7b")
                FileOutputStream(file).use { output ->
                    val buffer = ByteArray(4 * 1024) // or other buffer size
                    var read: Int
                    while (input.read(buffer).also { read = it } != -1) {
